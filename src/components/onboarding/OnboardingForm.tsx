...
export function OnboardingForm({ timezones, prefilledBusinessName, prefilledVertical }: OnboardingFormProps) {
  const { state, onSubmit } = useApiForm({
    endpoint: '/api/onboarding/tenant',
    successMessage: 'Business configured.',
    redirectTo: '/dashboard',
    buildBody: (formData) => ({ tenantName: String(formData.get('business_name') ?? ''), businessType: formData.get('vertical') ? String(formData.get('vertical')) : null, timezone: String(formData.get('timezone') ?? 'Europe/Rome') }),
  });
  const isSubmitting = state.status === 'submitting';
  const sectorOptions = LAUNCH_PRESETS.map((preset) => ({ value: preset.id, label: SECTOR_LABELS[preset.id] ?? preset.name }));
